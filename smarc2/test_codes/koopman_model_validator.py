...
class FullStatePlotterNode(Node):
    def __init__(self):
        ...
        # Initialize SAM 2 Simulation (Koopman)
        self.dt = 0.1
        try:
            self.poly = joblib.load('koopman_basis.pkl')
            self.operator = joblib.load('koopman_operator.pkl')
            self.get_logger().info("Successfully loaded Koopman model.")
        except Exception as e:
            self.get_logger().error(f"Failed to load Koopman models: {e}")
            raise e

        # Unified 12-DOF state vector [x, y, z, roll, pitch, yaw, vx, vy, vz, wx, wy, wz]
        ...
        self.get_logger().info("Subscribed to Odometry and Control Feedbacks for SAM 1. Simulating SAM 2 using Koopman.")
    ...
    def sim_timer_cb(self):
        """Steps the Koopman model forward using SAM 1's latest inputs."""
        if not self.sim_initialized:
            return

        current_time = time.time() - self.start_time
        
        # 1. Prepare data shapes for scikit-learn
        X_in = self.current_state.reshape(1, -1)
        U_in = np.array(self.latest_u).reshape(1, -1)
        
        try:
            # 2. Lift into Observable Space (EDMD)
            XU_in = np.hstack((X_in, U_in))
            Z_in = self.poly.transform(XU_in)
            
            # 3. Predict next state mapping
            raw_prediction = self.operator.predict(Z_in).flatten()
            
            # 4. CLAMP THE STATE: Prevent numerical explosion
            # Limit positions to +/- 1000m, angles/velocities to +/- 20
            self.current_state = np.clip(raw_prediction, -1000.0, 1000.0)
            
        except ValueError as e:
            self.get_logger().error(f"Koopman divergence detected: {e}. Resetting state from Unity.")
            self.sim_initialized = False  # Force a clean reset on the next Odom message
            return
            
        # Extract for logging
        eta = self.current_state[0:6]
        nu = self.current_state[6:12]
    ...
    def extract_odom_data(self, msg, data_dict):
        ...
        if data_dict is self.sam1_data and not self.sim_initialized:
            self.current_state = np.array([x, y, z, roll, pitch, yaw, vx, vy, vz, wx, wy, wz])
            self.sim_initialized = True
            self.get_logger().info(f"Koopman model initialized at [X:{x:.2f}, Y:{y:.2f}, Z:{z:.2f}]")
...
fig.canvas.manager.set_window_title('SAM: Actual (Unity) vs Koopman Dynamics')
...
def update_plot(frame, node):
    ...
    def plot_pair(ax, t_key, y_key, title, ylabel):
        if len(d1[t_key]) > 0:
            ax.plot(d1[t_key], d1[y_key], label='SAM 1 (Unity)', color='blue')
        if len(d2[t_key]) > 0:
            ax.plot(d2[t_key], d2[y_key], label='SAM 2 (Koopman)', color='red', linestyle='--')
    ...
    if len(d1['x']) > 0:
        ax_2d.plot(d1['x'], d1['y'], label='SAM 1 (Unity)', color='blue')
    if len(d2['x']) > 0:
        ax_2d.plot(d2['x'], d2['y'], label='SAM 2 (Koopman)', color='red', linestyle='--')
    ...
    if len(d1['x']) > 0:
        ax_3d.plot(d1['x'], d1['y'], d1['z'], label='SAM 1 (Unity)', color='blue')
    if len(d2['x']) > 0:
        ax_3d.plot(d2['x'], d2['y'], d2['z'], label='SAM 2 (Koopman)', color='red', linestyle='--')
