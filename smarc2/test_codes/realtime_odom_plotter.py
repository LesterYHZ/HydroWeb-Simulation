...
class FullStatePlotterNode(Node):
    def __init__(self):
        ...
        self.create_subscription(Odometry, '/sam_2/smarc/odom', self.sam2_odom_cb, 10)
        ...
        # Feedback Subscribers (SAM 2)
        self.create_subscription(Float32, '/sam_2/core/standard/lcg_fb', self.make_cb(self.sam2_data, 't_lcg', 'lcg'), 10)
        self.create_subscription(Float32, '/sam_2/core/standard/vbs_fb', self.make_cb(self.sam2_data, 't_vbs', 'vbs'), 10)
        self.create_subscription(Float32, '/sam_2/core/standard/thruster1_fb', self.make_cb(self.sam2_data, 't_t1', 't1'), 10)
        self.create_subscription(Float32, '/sam_2/core/standard/thruster2_fb', self.make_cb(self.sam2_data, 't_t2', 't2'), 10)
            
        self.get_logger().info("Subscribed to Odometry and Control Feedbacks for SAM 1 & 2")
...
fig.canvas.manager.set_window_title('SAM 1 & 2: Full State & Controls')
...
def update_plot(frame, node):
    ...
    ax_2d.plot(d2['x'], d2['y'], label='SAM 2', color='orange')
    ...
    ax_3d.plot(d2['x'], d2['y'], d2['z'], label='SAM 2', color='orange')
